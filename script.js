...
const els = {
  ...
  generatedVisual: document.querySelector("#generatedVisual"),
...
function canStartYear(year, count) {
  return count >= 50 || year === "2020";
}

function renderYearChoices() {
  els.yearSelect.innerHTML = '<option value="">Select a year</option>';
  availableYears().forEach((year) => {
    const tests = fullYearTests();
    const count = tests[year] ? tests[year].length : questionBank.filter((question) => question.year === year).length;
    const startable = canStartYear(year, count);
    const option = document.createElement("option");
    option.value = year;
    option.textContent = `${year} (${count}/50)`;
    option.disabled = !startable;
    if (!startable) {
...
function chooseYear(year) {
  const tests = fullYearTests();
  if (tests[year] && !canStartYear(year, tests[year].length)) return;
  selectedYear = year;
...
function imageUrl(path) {
  if (typeof embeddedImages !== "undefined" && embeddedImages[path]) {
    return embeddedImages[path];
  }
  return new URL(path.replace(/\\/g, "/"), window.location.href).href;
}

function visualTypeFor(item) {
  const text = `${item.question} ${item.category || ""}`.toLowerCase();
  if (text.includes("family tree") || text.includes("generations")) return "family";
  if (text.includes("map") || text.includes("island") || text.includes("caribbean")) return "map";
  if (text.includes("pie chart") || text.includes("graph") || text.includes("percentage")) return "chart";
  if (text.includes("cardinal") || text.includes("direction") || text.includes("compass")) return "compass";
  if (text.includes("recycled") || text.includes("recycling")) return "recycle";
  if (text.includes("worship") || text.includes("mosque")) return "worship";
  if (text.includes("report") || text.includes("caption") || text.includes("burnt")) return "report";
  if (text.includes("communication") || text.includes("cyber")) return "message";
  if (text.includes("worker") || text.includes("services")) return "workers";
  return "study";
}

function renderGeneratedVisual(item) {
  const type = visualTypeFor(item);
  const visual = {
    family: `
      <div class="visual-title">Family Tree</div>
    ...
    `,
    map: `
      <div class="visual-title">Caribbean Map</div>
      <div class="mini-map">
        <span class="land land-a"></span><span class="land land-b"></span><span class="land land-c"></span>
        <em>A</em><strong>B</strong><i>C</i>
      </div>
    `,
    chart: `
      <div class="visual-title">Read The Chart</div>
      <div class="mini-chart"><span></span><span></span><span></span><span></span></div>
    `,
    compass: `
      <div class="visual-title">Compass</div>
      <div class="compass-rose"><span>N</span><span>E</span><span>S</span><span>W</span></div>
    `,
    recycle: `
      <div class="visual-title">Recycling Sign</div>
      <div class="recycle-symbol"><span></span><span></span><span></span></div>
    `,
    worship: `
      <div class="visual-title">House Of Worship</div>
      <div class="worship-place"><span></span><i></i><b></b></div>
    `,
    report: `
      <div class="visual-title">Read The Report</div>
      <div class="report-card">Important community report</div>
    `,
    message: `
      <div class="visual-title">Message</div>
      <div class="message-card"><span></span><span></span><span></span></div>
    `,
    workers: `
      <div class="visual-title">Community Workers</div>
      <div class="workers-card"><span></span><span></span><i></i></div>
    `,
    study: `
      <div class="visual-title">Study The Question</div>
      <div class="study-card"><span></span><span></span><span></span></div>
    `
  }[type];
  els.generatedVisual.innerHTML = visual;
}
...
function renderQuestion() {
  ...
  if (item.image) {
    ...
    els.openImageLink.classList.remove("hidden");
    els.generatedVisual.innerHTML = "";
    els.generatedVisual.classList.add("hidden");
    ...
    els.openImageLink.classList.add("hidden");
...
function chooseAnswer(index, button) {
  ...
  if (index === item.correct) {
    ...
    els.badge.textContent = "Correct";
    playTone("correct");
    scheduleNextQuestion(850);
  } else {
    streak = 0;
    coins = Math.max(0, coins - 5);
    button.classList.add("wrong");
    buttons[item.correct].classList.add("correct");
    els.feedback.textContent = `Incorrect. You lost 5 coins. The correct answer is ${item.answers[item.correct]}. Next question in 5 seconds.`;
    els.badge.textContent = "Review answer";
...
function scheduleNextQuestion(delay) {
  ...
  advanceTimer = setTimeout(() => {
    advanceTimer = undefined;
    if (!screens.game.classList.contains("hidden") && answered) {
      nextQuestion();
    }
  }, delay);
}
...
els.yearSelect.addEventListener("change", (event) => chooseYear(event.target.value));
els.playAgainBtn.addEventListener("click", startGame);
els.homeBtn.addEventListener("click", goHome);
els.chooseYearBtn.addEventListener("click", goHome);
